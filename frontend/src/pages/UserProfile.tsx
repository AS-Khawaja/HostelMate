...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Fetch user profile
        const profileResponse = await fetch(`http://localhost:5000/api/hostels/user/${userId}`);
        ...
        const bookingsResponse = await fetch(`http://localhost:5000/api/hostels/bookings`);
        ...
        const wishlistResponse = await fetch(`http://localhost:5000/api/hostels/wishlist/${userId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const userId = localStorage.getItem('userId');
      const response = await fetch(`http://localhost:5000/api/hostels/user/${userId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDeleteBooking = async (bookingId: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/hostels/bookings/${bookingId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        }
